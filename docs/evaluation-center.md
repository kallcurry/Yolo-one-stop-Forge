...
| 阶段 | 内容 | 验收标准 |
| --- | --- | --- |
| 1 引擎与数据 | `evaluation_job` + 注册表 + `evaluation_runner`（纯模型层） | 单元测试通过；用现有模型 + 测试批次跑通一次子进程评估，产出符合契约的 `evaluation_result.json` |
| 2 评估中心 UI | 顶部第四模块激活（任务中心/新建/监控/结果） | GUI 上完成一次"选模型→选测试→跑完→看结果"全流程 |
