...
- 支持联动删除：删除图片重复副本时，可同步删除已确认重复的对应 JSON 标注与 TXT 标签副本；保留副本与未确认文件不会被删除，会跳过并提示原因。
- 一次扫描还输出**孤儿文件**（同批次目录中找不到对应图片的标注/标签，可移出到备份目录或删除到回收站）与**同名多版本**（同一文件名在多个目录各有一份且内容不同，可选择保留版本，其余移出备份或删除），避免“重复残留”和同名版本错配。
...
- 可选测试集划分：按比例逐来源抽取测试样本（类别感知、稀有类保护），写入独立测试批次 `test_data/<测试批次名>/`（含 `dataset.yaml` 与 `test_manifest.json`），训练批次中永不混入测试样本，为评估中心提供数据基础。
- 默认创建独立副本，也可配置其他写入策略。
